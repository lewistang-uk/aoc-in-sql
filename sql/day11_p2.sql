...
-- part one style approach with extra stipulations on the final count takes too long. DP probably needed.
-- split the lines on :
...
-- filter for paths which end with 'out' and visit both 'fft' and 'dac'
SELECT COUNT(*) 
FROM bfs
WHERE next_node = 'out'
AND instr(visited, '/' || 'fft' || '/') <> 0
AND instr(visited, '/' || 'dac' || '/') <> 0
;